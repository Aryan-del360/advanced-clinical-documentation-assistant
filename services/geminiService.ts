...
// Lazily construct the client so we can support both Node env (process.env) and Vite client env (import.meta.env)
const getAiClient = () => {
    // Try Node-style env first (useful for a server/proxy)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const nodeKey = typeof process !== 'undefined' ? (process.env as any)?.API_KEY || (process.env as any)?.GEMINI_API_KEY : undefined;
    // Then try Vite client env (will be available when building with Vite if a VITE_ prefixed variable is provided)
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
